...
describe('theme resolver — resolveTokens("default")', () => {
  ...
  it('returns every contract key with a non-empty value — zero missing or undefined', async () => {
    ...
    // The parent contract declares 139 distinct CSS custom-property keys once
    ...
    // note for why this is 139 (the source palette's 93, the
    ...
    // generic-weight addendum, and the 10 header/nav literal-closure
    // promotions of the final no-literal pass), and not the 70 quoted in the
    // source design specification's summary prose (that figure undercounts its
    // own tables by one row apiece in the color and typography sections).
    ...
    expect(CONTRACT_TOKEN_NAMES.length).toBe(139);
