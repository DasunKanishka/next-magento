...
describe('theme resolver — resolveTokens("default")', () => {
  ...
  it('returns every contract key with a non-empty value — zero missing or undefined', async () => {
    ...
    // The parent contract declares 149 distinct CSS custom-property keys once
    ...
    // note for why this is 149 (the source palette's 93, the
    ...
    // promotions, and the 10 home literal-closure promotions of the home
    // styling pass), and not the 70 quoted in the source design
    // specification's summary prose (that figure undercounts its own tables
    // by one row apiece in the color and typography sections).
    ...
    expect(CONTRACT_TOKEN_NAMES.length).toBe(149);
