...
// Cut-off hour is backend-sourced content in production
// (`getStoreIdentity().deliveryPromise.cutoffHour`); these pure helpers take
// it as an explicit argument, so the test supplies a fixture value.
const TEST_CUTOFF_HOUR = 22;
...
describe('timeUntilCutoff', () => {
  it('returns the remaining hours and minutes before the cut-off', () => {
    ...
    const r = timeUntilCutoff(now, TEST_CUTOFF_HOUR);
